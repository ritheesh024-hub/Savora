    ...
    <div className="bg-gradient-to-br from-primary/10 via-background to-accent/20 rounded-3xl p-6 md:p-10 border shadow-2xl overflow-hidden relative">
      ...
      <div className="max-w-3xl relative z-10">
        ...
        <div className="flex flex-wrap gap-4 mb-10">
          <Badge
            variant="outline"
            className="px-4 py-2 rounded-full flex items-center gap-2 bg-white/50 backdrop-blur"
          >
          ...
          <Badge
            ...
            className="px-4 py-2 rounded-full flex items-center gap-2 bg-white/50 backdrop-blur"
          >
            <Coffee className="w-4 h-4 text-amber-800" /> {context.timeOfDay}
          </Badge>
          <Badge
            variant="outline"
            className="px-4 py-2 rounded-full flex items-center gap-2 bg-white/50 backdrop-blur"
        ...
        {recommendations.length > 0 && (
          <div className="grid md:grid-cols-2 gap-4 mt-10 animate-in fade-in slide-in-from-bottom duration-500">
            {recommendations.map((rec, idx) => (
              <Card
                key={idx}
                className="bg-white/80 backdrop-blur border-primary/20 hover:border-primary transition-colors cursor-pointer group"
              >
